...
<div class="d-flex flex-column gap-4"> <!-- force top-down stacking -->

<?php foreach ($accounts as $key => $label): ?>
  <div class="table-wrapper mb-4">
    <h3><?= $label ?> Deposits</h3>
    <table class="table table-striped" data-account="<?= $key ?>">
      ...
      <tbody>
        <?php
          $sql = "SELECT * FROM Deposits WHERE Account = '$key' ORDER BY DateAdded DESC";
          $res = $conn->query($sql);
        ...
        <tr data-id="<?= $row['ID'] ?>">
          ...
          <td class="Account">
            <select class="form-select form-select-sm account-dropdown" disabled>
