...
async function fetchSpendingChartData(start, end) {
  const res = await fetch(`/api/spending-chart?start=${start}&end=${end}`);
...
export default function SpendingChart() {
  ...
  useEffect(() => {
    if (start && end) {
      setError(null);
      setLoading(true);
      fetchSpendingChartData(start, end)
        .then((fetchedData) => {
          setData(fetchedData);
        })
        .catch((err) => {
  ...
  return (
    <Card>
      <CardContent>
        <Typography variant="h6">Spending Chart</Typography>
