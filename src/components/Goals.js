...
function Goals() {
  ...
  const handleFetchGoalData = async () => {
    ...
    try {
      const response = await fetch(`/api/goals-table?q=${encodeURIComponent(goalName)}`);
      ...
      <Box sx={{ maxWidth: 1400, mx: 'auto' }}>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <TextField
            ...
            sx={{ mr: 2 }}
          />
          <Button variant="contained" onClick={handleFetchGoalData}>
            Fetch Goal Data
          </Button>
        </Box>
        {goalData && (
            ...
            <CardContent>
              ...
              <Doughnut
                data={{
                  ...
                  datasets: [
                    {
                      data: [goalData.progress, goalData.goalAmount - goalData.progress],
        ...
        {loading ? (
          ...
          <Grid container spacing={3} alignItems="stretch">
            {goals.map((goal, idx) => {
              ...
              return (
                <Grid xs={12} md={4} key={goal.id || idx} sx={{ display: 'flex', minWidth: 0 }}>
